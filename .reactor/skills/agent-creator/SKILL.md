---
name: agent-creator
description: Create Reactor agent profiles via interactive Q&A.
---

# Agent Profile Creator

Create agent profiles stored as `config.json` in `~/.reactor/agents/{name}/` (global) or `{project}/.reactor/agents/{name}/` (project-level).
